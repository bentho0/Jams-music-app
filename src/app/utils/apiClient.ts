 ...
 *
 * Key invariant: isFigmaSandbox() is called FRESH on every request, not from
 * a module-level const. This matters because Figma's Service Worker isn't
 * "controlling" the preview page until after module evaluation completes.
 * A cached const would be false; calling isFigmaSandbox() at request time
 * catches the SW that is now active.
 */
...
export async function apiFetch(
  ...
  // ── Guard: re-evaluate every time, not from a stale module-level const ──
  if (isFigmaSandbox()) {
