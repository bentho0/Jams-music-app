...
export function AIPlaylistDisplay({
  ...
  // Return-flow toast state
  const [toastState, setToastState] = useState<ToastState>("hidden");
  const [toastVisible, setToastVisible] = useState(false); // controls CSS slide-in
  ...
  const handleRefine = useCallback(async () => {
    ...
    // Block network in Figma sandbox — worker throws before our catch() runs
    if (isFigmaSandbox()) {
    ...
    try {
      ...
      const newIds = new Set<string>(
        (data.tracks as Array<{ spotify_track_id: string }>)
          .filter(t => t.spotify_track_id && !oldIds.has(t.spotify_track_id)) // exclude empty-string IDs (AI fallbacks)
  ...
  // ── Sync livePlaylistData when parent pushes a brand-new playlist
  //    (Regenerate keeps the component mounted, so useState initial value
  //     never re-runs — this effect bridges the gap)
  useEffect(() => {
    console.log("[Jams] playlistData prop changed — syncing livePlaylistData");
    console.log("[Jams] New playlist title:", playlistData.playlist_title, "| tracks:", playlistData.tracks?.length);
    console.log("[Jams] First track full object:", JSON.stringify(playlistData.tracks?.[0], null, 2));
  ...
  // ── Persist playlist for sharing on mount ───────────────────────
  useEffect(() => {
    const id = shareId;
    const url = `${window.location.origin}/playlist/${id}`;
    setSharePersistedUrl(url);

    // Skip backend persistence in Figma's sandboxed preview — fetch is blocked there
  ...
  // visibilitychange listener for return detection
  useEffect(() => {
  ...
  const showCheckinToast = () => {
    ...
    // Allow state to paint before triggering fade-in
    requestAnimationFrame(() => {
      requestAnimationFrame(() => setToastVisible(true));
    });
    // No auto-dismiss — stays until the user interacts
  };
  ...
  // Sync displayTitle/displayDescription when AI refines the playlist
  useEffect(() => {
  ...
  const saveToAnyMusicApp = () => {
    const trackList = formatPlaylistForTunemymusic();
    // Auto-copy track list to clipboard
    try {
      navigator.clipboard.writeText(trackList).catch(() => {
        const el = document.createElement("textarea");
        el.value = trackList;
        el.style.cssText = "position:fixed;top:-9999px;left:-9999px;opacity:0;";
        document.body.appendChild(el);
  ...
  const handlePlayOnSpotify = async () => {
    // If not connected to Spotify, initiate OAuth flow
    if (!spotifySessionId) {
      ...
      // Page will redirect — no need to reset isConnecting
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        // If session expired, prompt re-connect
      ...
      console.log('Playlist saved to Spotify:', data.playlistUrl);
    } catch (error) {
  ...
  return (
    <div className="fixed inset-0 bg-[#0a0b0d] z-50 overflow-y-auto">
      {/* Close button — fixed top-right of screen, outside all containers */}
      <button
        onClick={onClose}
      ...
      <div className="max-w-5xl mx-auto px-5 md:px-6 py-10 md:py-12">
        ...
        <div className="relative mb-12 mt-3">
          ...
          <div className="bg-white/5 backdrop-blur-xl rounded-3xl p-5 md:p-10 border border-white/10">
            <div className="flex flex-col md:flex-row gap-6 md:gap-10 items-start">

              {/* ── Playlist Cover (2×2 mosaic) ── */}
              <div
              ...
              >
                ...
                {/* Cover image: custom upload OR auto mosaic */}
                {customCover ? (
                  ...
                  if (artworks.length < 4) {
                    return <img src={artworks[0]} alt="Playlist cover" className="w-full h-full object-cover" onError={(e) => { e.currentTarget.style.display = "none"; }} />;
                  }

                  return (
                    <div className="w-full h-full grid grid-cols-2 grid-rows-2">
                      {artworks.map((url, i) => (
                        <img key={i} src={url} alt="" className="w-full h-full object-cover" onError={(e) => { e.currentTarget.style.display = "none"; }} />
                      ))}
                    </div>
                  );
                })()}
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex flex-col md:flex-row md:items-start gap-5 md:gap-3">
                  ...
                  {/* Secondary — Share Playlist */}
                  <button
                    onClick={handleShareButton}
                    className="flex items-center justify-center gap-2 px-5 rounded-lg bg-white/10 hover:bg-white/20 text-white font-semibold transition-all border border-white/20 w-full md:w-auto" style={{height: '52px'}}
                  >
        ...
        <div className="bg-white/5 backdrop-blur-xl rounded-2xl border border-white/10 overflow-hidden">
          ...
          <div className="divide-y divide-white/10">
            {livePlaylistData.tracks.map((track, index) => {
              ...
              // Fallback tracks have spotify_track_id="" — using that as a key
              // gives every fallback the same key "", causing React to skip rendering
              // all but the first. Use a composite key to guarantee uniqueness.
                    ...
                    {isRefining ? (
                      ...
                      <img
                        ...
                        onError={(e) => {
                          ...
                          if (parent) {
                            parent.innerHTML = `<div class="w-full h-full flex items-center justify-center"><svg class="w-6 h-6" style="color:rgba(255,255,255,0.2)" fill="currentColor" viewBox="0 0 20 20"><path d="M18 3a1 1 0 00-1.196-.98l-10 2A1 1 0 006 5v9.114A4.369 4.369 0 005 14c-1.657 0-3 .895-3 2s1.343 2 3 2 3-.895 3-2V7.82l8-1.6v5.894A4.37 4.37 0 0015 12c-1.657 0-3 .895-3 2s1.343 2 3 2 3-.895 3-2V3z"/></svg></div>`;
                          }
                        }}
                      />
                    ) : (
        ...
        {/* Bottom padding — stops scroll when tracks are 48px above input bar */}
        <div className="h-12" />
      </div>
        ...
        <div className="max-w-5xl mx-auto px-5 md:px-6 pb-4">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {refinementHistory.map((item, i) => (
                    <button
                      ...
                      className="px-2.5 py-1 rounded-full bg-white/10 hover:bg-white/20 text-white/60 hover:text-white text-xs transition-all border border-white/10"
                    >
                      "{item}"
                    </button>
                  ))}
          ...
          <div
            className={`flex items-center gap-3 h-14 bg-[#1a1b20]/95 backdrop-blur-xl border rounded-2xl px-4 shadow-[0_8px_32px_rgba(0,0,0,0.6)] transition-all ${
              shakeInput ? "border-red-500/50 animate-[shake_0.3s_ease]" : "border-white/15 focus-within:border-[#4feec5]/50"
            }`}
          >
            {/* Sparkle icon */}
      ...
      {isEditModalOpen && (
        ...
        >
          ...
          {/* Modal */}
          <div className="relative w-full max-w-md bg-[#1a1b20] rounded-2xl shadow-2xl border border-white/10 overflow-hidden">
            {/* Header */}
            ...
            <div className="px-6 py-5 space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full bg-white/5 border border-white/10 rounded-lg px-4 py-3 text-white placeholder-white/30 text-sm focus:outline-none focus:border-[#4feec5]/60 focus:bg-white/8 transition-all"
                />
              </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full bg-white/5 border border-white/10 rounded-lg px-4 py-3 text-white placeholder-white/30 text-sm focus:outline-none focus:border-[#4feec5]/60 focus:bg-white/8 transition-all resize-none"
                />
            ...
            <div className="px-6 pb-6 flex justify-end gap-3">
              <button
                onClick={() => setIsEditModalOpen(false)}
                className="px-5 py-2.5 rounded-lg text-white/60 hover:text-white text-sm font-medium transition-colors"
              >
                Cancel
              </button>
              <button
      ...
      {showExportModal && (
        ...
        >
          ...
          {/* Modal */}
          <div className="relative w-full max-w-md bg-[#1a1b20] rounded-2xl shadow-2xl border border-white/10 overflow-hidden max-h-[90vh] flex flex-col">
            {/* Header */}
            ...
            <div className="px-6 py-5 space-y-5 overflow-y-auto">
              ...
              <div className="space-y-3.5">
                ...
                {[
                  { step: "1", label: <>Select <span className="text-white font-semibold">"Free Text"</span> as your source<br /><span className="text-white/40 text-xs">(It's the plain text / manual entry option)</span></> },
                  { step: "2", label: <>Paste your track list<br /><span className="text-white/40 text-xs">Press Ctrl+V on Windows or ⌘+V on Mac — it's already waiting in your clipboard</span></> },
                  { step: "3", label: <>Choose where to save it<br /><span className="text-white/40 text-xs">Spotify, Apple Music, YouTube Music, and more</span></> },
                  { step: "4", label: <>Hit <span className="text-white font-semibold">Transfer</span><br /><span className="text-white/40 text-xs">Your playlist will be built in seconds ✓</span></> },
                ].map(({ step, label }) => (
                  <div key={step} className="flex items-start gap-3">
                    <span className="w-6 h-6 rounded-full bg-white/10 text-white/50 text-xs font-bold flex items-center justify-center flex-shrink-0 mt-0.5">
                      {step}
                    </span>
              ...
              <p className="text-white/25 text-xs leading-relaxed border-t border-white/10 pt-4">
                Tunemymusic is free and takes under a minute — no account required
              </p>
            ...
            <div className="px-6 pb-6 flex flex-col-reverse sm:flex-row sm:justify-end gap-3 flex-shrink-0">
              <button
                onClick={() => setShowExportModal(false)}
                className="px-5 py-2.5 rounded-lg text-white/60 hover:text-white text-sm font-medium transition-colors sm:w-auto w-full text-center"
              >
                Cancel
              </button>
              <button
      ...
      {showShareModal && (() => {
        ...
        return (
          ...
          >
            ...
            <div
              className="relative w-full max-w-[460px] bg-[#1a1b20] border border-white/10 rounded-2xl shadow-2xl overflow-hidden"
              style={{
              ...
              <div className="px-6 py-5 space-y-5">
                ...
                <div>
                  ...
                  <textarea
                    ...
                    rows={2}
                    className="w-full bg-white/5 border border-white/10 rounded-lg px-4 py-3 text-white/80 text-sm leading-relaxed focus:outline-none focus:border-[#4feec5]/50 transition-all resize-none"
                  />
                </div>
                ...
                <div className="grid grid-cols-3 sm:grid-cols-5 gap-2">
                  {[
                    ...
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="flex flex-col items-center gap-1.5 px-2 py-3 rounded-xl bg-white/5 hover:bg-white/10 text-white transition-all border border-white/5 hover:border-white/20"
                    >
                      {icon}
                ...
                <button
                  ...
                  className="w-full flex items-center justify-center gap-2 px-4 py-3 rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 hover:border-white/20 text-white/80 text-sm font-medium transition-all"
                >
      ...
      >
        <div className="flex items-center gap-2 px-4 py-2.5 bg-[#1a1b20] border border-white/10 rounded-xl shadow-xl text-white text-sm font-medium whitespace-nowrap">
          <Check className="w-4 h-4 text-[#4feec5]" />
      ...
      {toastState !== "hidden" && (
        ...
        >
          ...
          <div
            ...
            onClick={() => dismissToast("fade")}
          />
          <div className="relative w-full max-w-[420px] bg-[#1a1b20] border border-white/10 rounded-2xl shadow-2xl overflow-hidden"
            style={{
          ...
          >
            ...
            {toastState === "checkin" && (
              <div className="p-5">
                <div className="flex items-start justify-between mb-4">
                  ...
                  <button
                    onClick={() => dismissToast("fade")}
                    className="ml-3 text-white/30 hover:text-white/70 transition-colors flex-shrink-0 -mt-0.5"
                  >
                    <X className="w-4 h-4" />
                  </button>
                </div>
                <div className="flex gap-3 flex-col sm:flex-row">
                  <button
                    ...
                    className="flex-1 flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg bg-[#4feec5] hover:bg-[#3fd9b5] text-[#0a0b0d] text-sm font-bold transition-all"
                  >
                    <Check className="w-4 h-4" />
                    Yes, it worked
                  </button>
                  <button
                    onClick={handleToastHelp}
                    className="flex-1 flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg border border-white/20 text-white/70 hover:text-white hover:border-white/40 text-sm font-medium transition-all"
                  >
                    <X className="w-4 h-4" />
                    I had trouble
                  </button>
            ...
            {toastState === "success" && (
              <div className="p-5">
                ...
                <div className="flex gap-3 flex-col sm:flex-row">
                  <button
                    onClick={() => { dismissToast("fade"); onClose(); }}
                    className="flex-1 flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg bg-[#4feec5] hover:bg-[#3fd9b5] text-[#0a0b0d] font-bold transition-all whitespace-nowrap text-[14px] text-center"
                  >
                    <RotateCcw className="w-4 h-4 shrink-0" />
                    Generate New Playlist
                  </button>
                  <button
                    onClick={() => { dismissToast("fade"); onEditPrompt(); }}
                    className="flex-1 flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg border border-white/20 text-white/70 hover:text-white hover:border-white/40 text-sm font-medium transition-all"
                  >
            ...
            {toastState === "help" && (
              ...
              >
                ...
                <div className="flex gap-3 flex-col sm:flex-row">
                  <button
                    onClick={() => { openTunemymusic(); dismissToast("fade"); }}
                    className="flex-1 flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg bg-[#4feec5] hover:bg-[#3fd9b5] text-[#0a0b0d] text-sm font-bold transition-all"
                  >
                    <ExternalLink className="w-4 h-4" />
                    Re-open Tunemymusic
