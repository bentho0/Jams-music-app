...
import { PlaylistGeneratingLoader } from "./PlaylistGeneratingLoader";
import { AIPlaylistDisplay } from "./AIPlaylistDisplay";
...
export function MainApp() {
  ...
  // Accumulates all Spotify track IDs ever generated in this session to prevent repeats
  const seenTrackIds = useRef<Set<string>>(new Set());
  // Accumulates human-readable track names for the AI exclusion hint (IDs are meaningless to OpenAI)
  ...
  const generatePlaylist = async (prompt: string, excludedTrackIds: string[] = []) => {
    ...
    try {
      ...
      console.log("[Jams] Full playlist object:", JSON.stringify(data, null, 2));
      console.log("[Jams] First track full object:", JSON.stringify(data.tracks?.[0], null, 2));

      // Add newly returned track IDs to the seen set
      ...
      // Clear loading before returning — the finally block won't do it on attempt 1
      ...
      // Always clear loading on a final error
      setIsGeneratingPlaylist(false);
      setIsRegenerating(false);
    }
    }

    // Fallback cleanup (should not normally be reached)
    setIsGeneratingPlaylist(false);
    setIsRegenerating(false);
  };
        ...
        <div className="fixed top-0 left-0 right-0 z-[100] flex items-center justify-between gap-3 px-4 py-2.5 bg-yellow-500/10 border-b border-yellow-500/20 backdrop-blur-sm">
          <div className="flex items-center gap-2.5 min-w-0">
            ...
            <p className="text-yellow-300 text-xs leading-snug">
              <span className="font-semibold">Figma preview detected</span> — network requests are blocked in this sandbox. Open the{" "}
              <span className="underline underline-offset-2">published app URL</span> in a browser tab to use Jams fully.
            </p>
          </div>
          <button
        ...
        return (
          <div className="fixed inset-0 bg-[#0a0b0d]/95 z-50 flex flex-col items-center justify-center gap-6 px-5">
            <div className={`w-14 h-14 rounded-full flex items-center justify-center mb-2 ${
              ...
              {isNetworkError ? (
                <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="#eab308" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                  <path d="M1 6s4-2 11-2 11 2 11 2"/><path d="M5 10s3-1 7-1 7 1 7 1"/><line x1="1" y1="1" x2="23" y2="23"/><path d="M16.72 11.06a10 10 0 0 1 2.28 2.94"/><path d="M5 12.55a10 10 0 0 1 5.17-2.39"/><path d="M10.71 5.05A16 16 0 0 1 22.56 9"/><path d="M1.42 9a15.91 15.91 0 0 1 4.7-2.88"/><path d="M8.53 16.11a6 6 0 0 1 6.95 0"/><line x1="12" y1="20" x2="12.01" y2="20"/>
                </svg>
              ) : (
                <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke={isTimeout ? "#f97316" : "#ef4444"} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
            ...
            <div className="text-center max-w-md">
              ...
              {isNetworkError && (
                <div className="mt-3 bg-yellow-500/10 border border-yellow-500/20 rounded-xl px-4 py-3 text-left">
                  <p className="text-yellow-400/80 text-xs font-semibold mb-1">Why does this happen?</p>
                  <p className="text-white/40 text-xs leading-relaxed">
                    Figma's preview sandbox blocks outgoing network requests. To test the full Jams experience, open the app's published URL in a regular browser tab.
                  </p>
                </div>
              )}
              {!isNetworkError && (
                <p className="text-white/30 text-xs mt-1">Check the browser console and Supabase Edge Function logs for details.</p>
              )}
            </div>
